...
export default function BlackHole(props: Props) {
    ...
    const outerRadFromSize = useCallback(
        (w: number, _h: number) => {
            ...
            const pct = Math.max(0, Math.min(100, outerRadius ?? 70)) / 100
    ...
    useEffect(() => {
        ...
        const draw = (now: number) => {
            ...
            const tiltRad = ((tilt ?? 20) * Math.PI) / 180
            const tiltSidewayRad = ((tiltSideway ?? 160) * Math.PI) / 180
            ...
            for (let i = 0; i < pts.length; i++) {
                ...
                const localOrbitSpeed =
                    (orbitSpeed ?? 4) * speedFactor * pt.speedOffset
                ...
                const activeColors = colors && colors.length > 0 ? colors : ["#ffffff"]
                const color = activeColors[pt.colorIdx % activeColors.length]
