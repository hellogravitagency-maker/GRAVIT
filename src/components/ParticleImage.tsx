...
function __OriginkitBase_ParticleImage(__props) {
    ...
    const initParticles = () => {
        ...
        let loadedSrcs = new Array(urls.length);
        let loadedCount = 0;
        ...
        const finalize = () => {
            const maxLen = Math.max(1, ...loadedSrcs.map(s => s ? s.length : 0));
            ...
            for (let i = 0; i < maxLen; i++) {
                const tgts = loadedSrcs.map(srcArr => {
                    if (!srcArr || srcArr.length === 0) return { homeX: W/2, homeY: H/2, r:0, g:0, b:0, a:0 };
                    return srcArr[i % srcArr.length];
                });
                
                const activeSrcIndex = morphStateRef.current % tgts.length || 0;
                const initialTgt = tgts[activeSrcIndex];
                const pt = mkParticle(initialTgt, initialTgt.homeX, initialTgt.homeY, initialTgt.homeX, initialTgt.homeY);
                pt.targets = tgts;
                ...
                if (ht === "roam" && hOn) {
                    ...
                    pt.x = rx; pt.y = ry; pt.startX = rx; pt.startY = ry;
                    const [tx, ty] = randomInShape(rs, bx, by, bw, bh)
                    pt.roamTargetX = tx; pt.roamTargetY = ty;
                    pt.vx = (Math.random() - 0.5) * 1.2
                    pt.vy = (Math.random() - 0.5) * 1.2
                } else if (hOn) {
                    const [ox, oy] = hidePos(initialTgt.homeX, initialTgt.homeY)
                    pt.x = ox; pt.y = oy; pt.startX = ox; pt.startY = oy;
                }
        ...
        urls.forEach((u, idx) => {
            const img = new Image()
            if (typeof u === "string" && u.startsWith("http")) img.crossOrigin = "anonymous"
            img.onerror = () => {
                loadedSrcs[idx] = [];
                loadedCount++;
                if (loadedCount === urls.length) finalize();
            }
            img.onload = () => {
                loadedSrcs[idx] = processImage(img, idx);
                loadedCount++;
                if (loadedCount === urls.length) finalize();
            }
            img.src = u;
    ...
    useEffect(() => {
        const { morphInterval } = samplingRef.current;
        if (!morphInterval || morphInterval <= 0) return;
        
        // Wait a bit before starting the morph loop so initial layout stabilizes
        const timeout = setTimeout(() => {
            const intervalId = setInterval(() => {
                const { particles } = sceneRef.current;
                if (!particles || !particles.length || !particles[0].targets) return;
                const numTargets = particles[0].targets.length;
                if (numTargets < 2) return;
                
                morphStateRef.current = (morphStateRef.current + 1) % numTargets;
                const nextIdx = morphStateRef.current;
                
                animStartTimeRef.current = Date.now();
                animStateRef.current = "assembling";
                
                const { hoverType: ht, hover: hOn } = physicsRef.current;
                
                for (const p of particles) {
                    p.startX = p.x;
                    p.startY = p.y;
                    const tgt = p.targets[nextIdx];
                    p.homeX = tgt.homeX;
                    p.homeY = tgt.homeY;
                    p.r = tgt.r;
                    p.g = tgt.g;
                    ...
                    if (ht === "roam" && hOn) {
                        // Keep roaming targets around the center but they will morph colors
                    }
                }
            }, morphInterval);
            
            samplingRef.current.morphIntervalId = intervalId;
        }, Math.min(1000, morphInterval / 2));
        
        return () => {
            clearTimeout(timeout);
            if (samplingRef.current.morphIntervalId) {
                clearInterval(samplingRef.current.morphIntervalId);
            }
        };
    }, [morphInterval])
...
export default function ParticleImage(props) {
  return <__OriginkitBase_ParticleImage {...(__originkitPresetProps)} {...props} />;
}
